...
class EnemyNode: SKSpriteNode {
    ...
    fileprivate func configureMovements(_ moveStraight: Bool) {
        let path = UIBezierPath()
        path.move(to: .zero)
        if moveStraight {
            path.addLine(to: .init(x: -10000, y: 0))
        } else {
            path.addCurve(to: .init(x: -3500, y: 0), controlPoint1: .init(x: 0, y: -position.y * 4), controlPoint2: .init(x: -1000, y: -position.y))
        }
